        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          ...
          >
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="w-32 h-32 md:w-48 md:h-48 rounded-[40px] bg-slate-100 flex items-center justify-center border-4 border-white shadow-xl overflow-hidden shrink-0">
                {personalInfo?.profilePhoto ? (
                  <img src={personalInfo.profilePhoto} alt={personalInfo.name} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-4xl md:text-6xl font-black text-slate-300">{personalInfo?.name?.charAt(0)}</span>
                )}
              </div>
              <div className="text-center md:text-left">
                <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-4 text-slate-950">
                  {personalInfo?.name}
                </h1>
                <p className="text-xl md:text-2xl font-bold text-indigo-600 mb-6">{personalInfo?.role}</p>
                <div className="flex flex-wrap justify-center md:justify-start gap-4">
                  {personalInfo?.email && (
                    <a href={`mailto:${personalInfo.email}`} className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-medium transition-colors">
                      <Mail className="w-4 h-4" /> {personalInfo.email}
                    </a>
                  )}
                  {personalInfo?.location && (
                    <span className="flex items-center gap-2 text-slate-500 font-medium tracking-tight">
              ...
              <div className="space-y-12">
                {experience?.length > 0 ? experience.map((exp, i) => (
                  <div key={i} className="relative pl-8 border-l-2 border-slate-100">
                    <div className="absolute top-0 left-[-9px] w-4 h-4 rounded-full bg-white border-2 border-indigo-600" />
                    ...
                    <div className="text-lg font-semibold text-slate-600 mb-4">{exp.company}</div>
            ...
            <div>
              ...
              <div className="space-y-12">
                {education?.length > 0 ? education.map((edu, i) => (
                  <div key={i} className="relative pl-8 border-l-2 border-slate-100">
                    <div className="absolute top-0 left-[-9px] w-4 h-4 rounded-full bg-white border-2 border-slate-400" />
                    <div className="mb-2 flex items-center gap-2 text-sm font-bold text-slate-400 uppercase tracking-wider">
                      <Calendar className="w-3 h-3" /> {edu.startYear} - {edu.endYear}
                    </div>
                    <h3 className="text-xl font-bold text-slate-900 mb-1">{edu.degree} in {edu.fieldOfStudy}</h3>
                    <div className="text-lg font-semibold text-slate-600 mb-4">{edu.institution}</div>
                    <p className="text-slate-500 leading-relaxed">{edu.description}</p>
                  </div>
                )) : (
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-10">
            {projects?.length > 0 ? projects.map((project, i) => (
              <div key={i} className="group p-8 rounded-[40px] bg-slate-50 border border-slate-100 hover:bg-white hover:border-indigo-100 transition-all hover:shadow-2xl hover:shadow-indigo-500/5 overflow-hidden relative">
                <div className="mb-6 flex justify-between items-start">
                  <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center border border-slate-100 shadow-sm group-hover:scale-110 transition-transform">
                    <Layers className="w-8 h-8 text-indigo-600" />
                  </div>
                  <div className="flex gap-3">
                    {project.githubLink && (
                      <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="p-3 bg-white rounded-full border border-slate-100 hover:text-indigo-600 transition-colors shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.techStack?.map((tech, j) => (
                    <span key={j} className="px-4 py-2 bg-white rounded-full text-xs font-bold text-slate-500 border border-slate-100 group-hover:border-indigo-100 group-hover:text-indigo-600 transition-colors">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <p className="text-slate-400 italic col-span-full">No projects added yet.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <footer className="py-20 border-t border-slate-100 bg-slate-50/30">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="flex gap-4">
            {socialLinks?.github && (
              <a href={socialLinks.github} target="_blank" rel="noopener noreferrer" className="p-4 bg-white rounded-full border border-slate-100 hover:text-indigo-600 transition-all hover:scale-110 shadow-sm">
                <Github className="w-6 h-6 text-slate-900" />
              </a>
            )}
            {socialLinks?.linkedin && (
              <a href={socialLinks.linkedin} target="_blank" rel="noopener noreferrer" className="p-4 bg-white rounded-full border border-slate-100 hover:text-indigo-600 transition-all hover:scale-110 shadow-sm">
                <Linkedin className="w-6 h-6 text-slate-900" />
              </a>
            )}
            {socialLinks?.twitter && (
              <a href={socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="p-4 bg-white rounded-full border border-slate-100 hover:text-indigo-600 transition-all hover:scale-110 shadow-sm">
